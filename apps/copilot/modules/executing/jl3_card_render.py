...
def render_inline_help_btn(help_html: str, *, title: str = "说明") -> str:
    ...
    if not help_html:
        return ""
    return f"""
<details class="inline-block relative align-middle ml-0.5" onclick="event.stopPropagation()">
  <summary class="list-none cursor-pointer w-4 h-4 rounded-full bg-gray-100 text-gray-500 text-[10px] font-bold inline-flex items-center justify-center hover:bg-blue-100 hover:text-blue-700 select-none" title="{_esc(title)}">?</summary>
...
def render_fii_secondary_collapsed(
    *,
    ...
    return f"""
<details class="mt-3 border border-gray-200 rounded-lg bg-gray-50/50" onclick="event.stopPropagation()">
  <summary class="cursor-pointer list-none px-3 py-2 text-[11px] text-gray-600 hover:bg-gray-100 [&::-webkit-details-marker]:hidden">
    合并总营收参考 · 发令规则说明 ▾
  </summary>
...
def render_jl3_strategy_collapsible(
    ...
    *,
    ...
    return f"""
<details class="mt-4 border border-blue-100 rounded-lg bg-blue-50/30 overflow-hidden group/jl3-{group_id}" onclick="event.stopPropagation()">
  <summary class="cursor-pointer list-none px-3 py-2.5 flex flex-wrap items-center justify-between gap-2 hover:bg-blue-50/80 [&::-webkit-details-marker]:hidden">
